"""src/streaming/consumer_foster.py.
...
def process_message(row: dict[str, Any]) -> dict[str, Any]:
    """Add derived fields for downstream analytics."""
    ...
    # Derived metric
    row["total_price"] = unit_price * quantity

    # Normalized region field
    row["region_clean"] = row.get("region_id", "").lower()

    # Optional: add processing metadata
    row["processed_by"] = "consumer_foster_pipeline"
